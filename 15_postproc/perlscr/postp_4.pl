...
our $struct    = "/home/miles/Projects/03_struct/struct"; # *********** changed by Mile
#my $dbdir     = "dbis"; 
our $dbdir     = "dbis"; # *********** changed by Mile
#my $pdbdir    = "pdbs";
our $pdbdir    = "pdbs"; # *********** changed by Mile
...
my $db_search = $ARGV[0]; # *********** changed by Mile
our $params_file = $ARGV[1]; # *********** changed by Mile
my $top_number = $ARGV[2]; # *********** changed by Mile
...
#### MULTITHREADING ***********inserted by Mile

my $THREADS= 16; # Number of threads

my $workq = Thread::Queue->new(); # Work to do

$workq->enqueue(@top_list); # Queue up some work to do
$workq->enqueue("EXIT") for(1..$THREADS); # And tell them when
                                          # they're done

threads->create("run_struct") for(1..$THREADS); # Spawn our workers


while(threads->list(threads::running)){	
}

#### END of MULTITHREADING *********** end inserting by Mile

# ***********inserted by Mile
foreach $hit (@top_list ) {

    ($qry_name, $tgt_name) = split " ", $hit;
	 $outname = $qry_name . "_" . $tgt_name ;
	`grep -v done $outname.struct_out >>  postp_out`;
	`rm $outname.struct_out`;

}

# *********** end inserting by Mile
...
sub run_struct 
{
	while(my $hit=$workq->dequeue()) {
    last if $hit eq 'EXIT'; # All done


#	($qry_name, $tgt_name) = split " ", $hit;
	my ($qry_name, $tgt_name) = split " ", $hit; # *********** changed by Mile

    # locate dbfiles
    my $tgt_db = `ls $dbdir/$tgt_name.db`; chomp $tgt_db;
#	print $tgt_db;
    my $qry_db = `ls $dbdir/$qry_name.db`; chomp $qry_db;

    # locate pdbfiles    <<<<<<
    my $tgt_pdb_name = substr $tgt_name, 0, 4;
    my $qry_pdb_name = substr $qry_name, 0, 4;
    #$tgt_pdb_name = substr $tgt_name, 0, 7;
    #$qry_pdb_name = substr $qry_name, 0, 7;

    my $tgt_pdb = "$pdbdir/$tgt_pdb_name.pdb";
    my $qry_pdb = "$pdbdir/$qry_pdb_name.pdb";

    if (  -z  $tgt_pdb) {
	`rm $tgt_pdb`;


	my $cmd = "/home/ivanam/perlscr/downloading/pdbdownload.pl  $tgt_pdb_name";
	(system $cmd) && die "error running $cmd\n";
    }
    if (  -z  $qry_pdb) {
	`rm $qry_pdb`;
    ...
    # to tgt and $qry
    my $tgt_chain = substr $tgt_name, 4, 1;
    my $qry_chain = substr $qry_name, 4, 1;

  #  $new_params = $params_file.".new";
	my $new_params = $params_file . "_" . $tgt_name;  # *********** changed by Mile
...
#=cut
    ...
    # run struct
   	...
   	#$outname = $qry_name;
    my $outname = $qry_name . "_" . $tgt_name ; # *********** changed by Mile
