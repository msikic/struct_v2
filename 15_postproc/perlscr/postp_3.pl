...
foreach $hit (@top_list ) {
   ...
   # $new_params = $params_file.".new";
	$new_params =  $params_file . "_" . $tgt_name;
    ...
    #$outname = $qry_name;
	my $outname = $qry_name . "_" . $tgt_name ;
...
foreach $hit (@top_list ) {
	...
	# changed by Mile
	`mv $tfmd_pdb pdbs`;
	my $tfmd_pdb_path = "pdbs/" . $tfmd_pdb;
	$pml_string .= "load $tfmd_pdb_path, $tgt_pdb_id\n";
