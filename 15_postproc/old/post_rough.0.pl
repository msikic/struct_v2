...
(@ARGV >= 2) ||
    die "Usage: !0  <tgt db> <qry db> <params> \n";
...
my ($tgt_db, $qry_db, $params_file) = @ARGV;
...
foreach ( $extract_region,  $struct,  $params_file,
...
`grep -v postp $params_file | grep -v verbose | grep -v outn > $params_file.tmp`;
foreach my $option ( "postp", "verbose") {
    `echo $option >> $params_file.tmp `;
}
...
($struct_out, $struct_tfm) = ("$qry_name.long_out", "$qry_name.struct_out.for_postp");
...
my (%CAs);
my ($ret, $key);
my ($num, $sse_id, $from, $to, $pdb, $chain);
my $line;
...
while ( ($key, $region) = each (%region) ) {
    ...
    $cmd = "$extract_region  $pdb -c $chain  $from  $to ".
	" | awk \'\$3==\"CA\"\'";
    $ret = `$cmd`;
    $CAs{$key} =  $ret;
}
...
# tgt should be translated
# and qry should be translated and rotated
my @qry_coords;
my @tgt_coords;
my (@r, @t, @T, @r_aux);
...
foreach $qry_ctr ( 0 .. $dim1-1) {
	...
	$similarity[$qry_ctr][$tgt_ctr] = exp(-$d);
    }
}
...
$pml_string .= "create $qry_pdb_id\_cpy, $qry_pdb_id\n";
$pml_string .= "create $tgt_pdb_id\_cpy, $tgt_pdb_id\n";

$pml_string .= "color tv_blue, $qry_pdb_id\n";
$pml_string .= "color raspberry, $tgt_pdb_id\n";

$pml_string .= "color bluewhite, $qry_pdb_id\_cpy\n";
$pml_string .= "color lightpink, $tgt_pdb_id\_cpy\n";

$pml_string .= "set cartoon_transparency, 0.5, $qry_pdb_id\_cpy\n";
$pml_string .= "set cartoon_transparency, 0.5, $tgt_pdb_id\_cpy\n";
...
$pml_string .= "show cartoon, matched_set or $qry_pdb_id\_cpy or  $tgt_pdb_id\_cpy\n";
$pml_string .= "deselect\n";
