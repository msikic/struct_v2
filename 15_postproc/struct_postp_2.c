...
# endif
	...
	/****************************************************/
	/* now try to extend the match to the previous loop */
...
# if 0
	printf ("*  %2d  %3s %2c ", resctr1, protein2->sequence[resctr1].pdb_id, 
		protein2->sequence[resctr1].res_type_short);
	if ( resctr2 >= 0 ) {
	    printf ("*  %2d  %3s  %2c ", resctr2, protein2->sequence[resctr2].pdb_id,
		    protein1->sequence[resctr2].res_type_short);
	}
	printf ("\n");
# endif
